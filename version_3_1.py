...
class Window_test(QtWidgets.QMainWindow):
    ...
    def openfiles(self):
        ...
        self.mdf_file = MDF(filename, memory="minimum")   # 第一个mdf文件作为下拉列表的条件信号
        for filename in filenames:
            mdf = MDF(filename, memory="minimum")
            signalnames = list(mdf.channels_db.keys())
            signals = [signal + "__" + filename for signal in signalnames]
            self.all_signals.addItems(signals)
    ...
    def save_configs(self):
        ...
        #     configs["selected_signals"].append(self.selected_signals.item(i).text().split("__")[0])
    ...
    def load_configs(self):
        ...
        if filename:
            ...
            if "checkbox_1" in configs.keys():
                ...
                self.add_condition_combox.setItemText(0, configs["condition_signal"])
                self.condition_combox.setItemText(0, configs["condition"])
            ...
            if "checkbox_2" in configs.keys():
                ...
                self.add_condition_combox_2.setItemText(0, configs["condition_signal_2"])
                self.condition_combox_2.setItemText(0, configs["condition_2"])
    ...
    def to_csv(self):
        ...
        if self.check_flag_1 == 1 and self.check_flag_2 == 0:
            ...
            for i in range(count):
                mdf_file = self.selected_signals.item(i).text().split("__")[1]
                signal_name = self.selected_signals.item(i).text().split("__")[0]
                mdf = MDF(mdf_file, "minimum")
